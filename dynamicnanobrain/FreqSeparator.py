...
def freqeuncy_signal_generator(tend,tnoise,fmin,fmax,dT,res=10) :
    # determine the size of the sequence
    dt = fmax**-1/res
    N = int(tend/dt)
    N1 = int((tend-tnoise)/dt) # steps of total time interval
    dN = int(dT/dt) # steps of average period
    # From the info above we can setup our intervals
...
def freqeuncy_single_step_generator(tend,fmin,fmax,res=10) :
    # determine the size of the sequence
    dt = fmax**-1/res
    N = int(tend/dt) # steps of total time interval
    Nhalf=int(N/2)
    frequency_control = np.zeros((N,1))
    frequency_control[:Nhalf] = fmin
    frequency_control[Nhalf:] = fmax
    frequency_output = np.zeros((N,1))
    z = 0
    for i in range(N):
        z = z + 2*np.pi*frequency_control[i]*dt
...
def train_network(network,teacher_signal,Tfit=300,beta=10) :
    ...
    # Fit output weights
    teacher_train = teacher_signal(tseries_train)
...
def characterize_network(network, tseries_train, pred_train, 
    ...
    teacher_test = teacher_signal(tseries_test)
...
def plot_timetrace(tseries_train,pred_train,tseries_test=None,pred_test=None,teacher_handle=None,
    ...
    ax.plot(tseries_train,teacher_handle(tseries_train),'--')
    if tseries_test is not None :
        ax.plot(tseries_test[:],pred_test[:])
        ax.plot(tseries_test,teacher_handle(tseries_test),'--')
    
    ax.set_xlabel('Time (ns)')
    ax.set_ylabel('Output signal (nA)')
    ax.set_ylim(0,pred_train.max()+50)
    
    plotter.save_plot(fig,plotname,PLOT_PATH)
    #plt.close()
...
def run_test(df=0.01,fmin=0.1,dT=50,Tfit=300,Tpred=300,Tnoise=100,Nreservoir=100,sparsity=0.9,
             ...
             teacher_scaling=1.0, memory_variation=0.0,RC_scale=1.0) :
    ...
    return train_error, pred_error, signal_diff_sq, noise_var
    ...
    for i in range(param_dict['n']):
        ...
        try :
            ...
            for m in range(N) :
                ...
                train_errors[m,i], pred_errors[m,i], signal_diffs[m,i], noise_vars[m,i] = run_test(plotname=plotname,**kwargs)
...
#%%
T1=600
train_error, pred_error, snr, noise = run_test(0.02,Tfit=T1,Tpred=T1,Tnoise=100,fmin=0.3,generate_plots=True,RC_scale=10)
...
ax.set_title('f0=0.3 GHz, df=2 MHz, 100 runs')
ax.set_ylim(0,12)
...
fig, _, _ = plotter.bode_multi_plot(devices,indicate_freq=0.3,plot_f_min=1e-2)
fig.show()
...
if True :
    ...
    ax1.plot(tseries[:Nmax],frequency_control[:Nmax])
    ax2.plot(tseries[:Nmax],frequency_output[:Nmax])
    #ax3.plot(frequency_control[:1000])
