...
# Extra node to connect recurrent signal back to layer 1
...
# Backwards connection from third layer to first hidden layer
weights['hd2->hd0'] = nw.connect_layers(3, 1, layers, channels)
# Recurrent connection
weights['out->hd2'] = nw.connect_layers(4, 3, layers, channels)
...
# Loop connections
weights['hd0->out'].connect_nodes(0 ,0 , channel='blue', weight=prop_weight)
weights['hd2->hd0'].connect_nodes(0 ,0 , channel='blue', weight=prop_weight)
# Add damping connection
weights['out->hd2'].connect_nodes(0 ,0 , channel='blue', weight=prop_weight)    
...
layers[3].Vthres=1.2 # loop excitation node
...
t_blue = [(3.0,4.0)] #, (8.0,8.5), (9.0,9,5), (10.0,11.0), (20.0,21.0), (30.0,31.0)] # 
...
layers[0].set_input_func(channel='blue',func_handle=physics.square_pulse, func_args=(t_blue, 4.0*Imax))
...
# %% Setup the recurrent feedback
layers[4].set_teacher_delay(3.0) # units of ns
...
plotter.visualize_dynamic_result(result,['I1-Iout-blue','H0-Iout','O0-Iout-red','L0-Iout'])
